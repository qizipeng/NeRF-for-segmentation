...
def load_data_split(basedir, scene, split, skip=1, try_load_min_depth=True, only_img_files=False, IsSegmantic = False):
    ...
    color_files = find_files('{}/color'.format(split_dir), exts=['*.png', '*.jpg'])
    if len(img_files) > 0:
        logger.info('raw img_files: {}'.format(len(img_files)))
        color_files = color_files[::skip]
        assert (len(color_files) == cam_cnt)
    else:
        color_files = [None, ] * cam_cnt
    # print(img_files)
    ...
    for i in range(cam_cnt):
        ...
        ray_samplers.append(RaySamplerSingleImage(H=H, W=W, intrinsics=intrinsics, c2w=pose,
                                                  ...
                                                  max_depth=max_depth, IsSegmantic= IsSegmantic))
