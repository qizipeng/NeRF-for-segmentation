...
class RaySamplerSingleImage(object):
    def __init__(self, H, W, intrinsics, c2w,
                       ...
                       mask_path=None,
                       min_depth_path=None,
                       max_depth=None, IsSegmantic = False):
        super().__init__()
        self.W_orig = W
        self.H_orig = H
        ...
        for x in range(0,512,32):
            for y in range(0,512,32):
                ...
                for i in range(0,32):
                    for j in range(0,32):
                        tmp.append((self.W * (x+i) + y+j))
                        self.all_sample.append((self.W * (x+i) + y+j))
                self.all_patch.append(tmp)
    ...
    def set_resolution_level(self, resolution_level):
        if resolution_level != self.resolution_level:
            ...
            if self.img_path is not None:
                if self.IsSegmantic:
                    self.img = imageio.imread(self.img_path).astype(np.float32)
            ...
            if self.color_path is not None:
                self.color = imageio.imread(self.color_path).astype(np.float32)/255.
                # self.img = cv2.resize(self.img, (self.W, self.H), interpolation=cv2.INTER_AREA)
                self.color = self.color[:, :, 0:3]
                self.color = self.color.reshape((-1, 3))
    ...
    def random_sample(self, N_rand, center_crop=False):
        ...
        if self.color is not None:
            color = self.color[select_inds, :]          # [N_rand, 3]
        else:
            color = None
    ...
    def random_patchsample(self, N_rand, center_crop=False):
        '''
        :param N_rand: number of rays to be casted
        :return:
        '''
        ...
        if center_crop:
            ...
            # x = np.random.randint(low=0, high=self.H - 32 * 1)
            # y = np.random.randint(low=0, high=self.W - 32 * 1)
            # select_inds = []
            # for i in range(0, 32):
            #     for j in range(0, 32):
            #         select_inds.append((self.W * (x + 1 * i) + y + 1 * j))
            samples = list(range(0,len(self.all_patch)))
            indx = random.choice(samples)
            select_inds = self.all_patch[indx]
        ...
        if self.img is not None:
            rgb = self.img[select_inds, :]          # [N_rand, 1]
        else:
            rgb = None
    ...
    def random_allpatchsample(self, center_crop=False):
        '''
        :param N_rand: number of rays to be casted
        :return:
        '''
        if center_crop:
            ...
            #     for j in range(0, 32):
            #         select_inds.append((self.W * (x + 1 * i) + y + 1 * j))
            select_inds = self.all_sample
            # select_inds = select_inds[1024:2048]
