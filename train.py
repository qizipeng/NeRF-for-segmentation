...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size):
    ...
    # ray_batch = ray_sampler.random_patchsample(1024)
    ...
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        color = ray_batch_split["color"][s]

        # colordd = Image.fromarray((color.cpu().reshape([32, 32, 3]).numpy() * 255).astype(np.uint8))
        # colordd.save("vis/"+str(s)+ "color.png")
        ...
        for m in range(models['cascade_level']):
            ...
            with torch.no_grad():
                ret = net(ray_o, ray_d, fg_far_depth, fg_depth, bg_depth, color=color, m=m)
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        optim = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=args.lrate)  ##freeze optim
        step_lr = StepLR(optim, step_size=args.updatesteps, gamma=0.1)
        models['net_{}'.format(m)] = net

        models['optim_{}'.format(m)] = optim
        models['step_lr_{}'.format(m)] = step_lr
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        for m in range(models['cascade_level']):
            for name in ['net_{}'.format(m)]:
                models[name].load_state_dict(to_load[name], strict=False)
    #start = 0
    return start, models
...
def ddp_train_nerf(rank, args):
    ...
    if torch.cuda.get_device_properties(rank).total_memory / 1e9 > 14:
        ...
        args.N_rand = 1024
        args.chunk_size = 8192 #1024  # 8192
    else:
    ...
    ray_samplers = load_data_split(args.datadir, args.scene, split='train',
                                   try_load_min_depth=args.load_min_depth, IsSegmantic= args.IsSegmantic)
    ...
    for global_step in range(start + 1, start + 1 + args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            net = models['net_{}'.format(m)]
            step_lr = models['step_lr_{}'.format(m)]
            # sample depths
            N_samples = models['cascade_samples'][m]
            if m == 0:
                # foreground depth
            ...
            ret = net(ray_batch['ray_o'], ray_batch['ray_d'], fg_far_depth, fg_depth, bg_depth,
                      color=ray_batch["color"], m=m)
            all_rets.append(ret)

            rgb_gt = ray_batch['rgb'].to(rank)

            if args.IsSegmantic and args.pointclass:
                rgb_gt = rgb_gt.squeeze()
                # print(ret['rgb'].shape, ret['color_class'].shape)
                rgb_loss = CrossEntropyLoss(ret['rgb'], rgb_gt.long())
                color_loss = CrossEntropyLoss(ret['color_class'], rgb_gt.long())
                loss = rgb_loss + color_loss
            if args.IsSegmantic and not args.pointclass:
                rgb_gt = rgb_gt.squeeze()
                # print(ret['rgb'].shape, ret['color_class'].shape)
                rgb_loss = CrossEntropyLoss(ret['rgb'], rgb_gt.long())
                loss = rgb_loss
            if not args.IsSegmantic:
...
def config_parser():
    ...
    parser.add_argument("--IsSegmantic", action='store_true')
    parser.add_argument("--Finetune", action='store_true')
    parser.add_argument("--pointclass", action='store_true')
    #parser.add_argument("--Fintune", action='store_true')
    parser.add_argument("--updatesteps", type=int, default=20000)
    parser.add_argument("--nclass", type=int, default=18)
    parser.add_argument("--color_channel", type=int, default=64)
